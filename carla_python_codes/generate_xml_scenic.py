...
while True:
    ...
    frame_path = 'output/scenic/%06d' % image.frame
    frame_path_xml = 'output/scenic/xml/%06d' % image.frame
            ...
            # to limit to drawing bounding boxes IN FRONT OF THE CAMERA
                ...
                if forward_vec.dot(ray) > 1:
                    ...
                    if npc.id in drone_civilian_ids:  
                        ...
                        if x_min > 0 and x_max < image_w and y_min > 0 and y_max < image_h:
                            writer.addObject('civilian_drones', x_min, y_min, x_max, y_max)
                    else:
                        if x_min > 0 and x_max < image_w and y_min > 0 and y_max < image_h:
                            writer.addObject('military_drones', x_min, y_min, x_max, y_max)
