...
weather = carla.WeatherParameters(
        sun_altitude_angle=random.randint(-10,50),
        precipitation=random.randrange(0,100,25),
        wind_intensity=random.randrange(0,100,25))
...
print(traffic_light_color[color])
...
while True:
    ...
    frame_path = 'output/traffic_lights/'+map+'_%06d' % image.frame
    frame_path_xml = 'output/traffic_lights/xml/'+map+'_%06d' % image.frame
    ...
    for bb in bounding_box_set_traffic_lights:
        ...
        if bb.location.distance(vehicle.get_transform().location) < 50:
            ...
            if forward_vec.dot(ray) > 1:
                ...
                if x_min<=0: x_min ==0
                if x_max>=image_w: x_max ==image_w
                if y_min<=0: y_min ==0
                if y_max>=image_h: y_max ==image_h
                ...
                if x_min >= 0 and x_max <= image_w and y_min >= 0 and y_max <= image_h:
                    if bb.location.distance(visible_traffic_light)<28:
                        ...
                        writer.addObject(color+'_traffic_light', x_min, y_min, x_max, y_max)
    ...
    writer.save(frame_path_xml + '.xml')
    cv2.imshow('ImageWindowName',img)
