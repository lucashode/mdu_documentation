...
def connect(world,camera_bp,image_queue,image_depth_queue,image_segmentation_queue,timeout = 5):
    t = time.time()
    while time.time()-t < timeout:
        try:
            ...
            camera_segmentation = world.spawn_actor(camera_segmentation_bp, camera_init_trans, attach_to=vehicle)
            
            camera.listen(image_queue.put)
            camera_segmentation.listen(image_segmentation_queue.put)
            camera_depth.listen(image_depth_queue.put)
            ...
            return id,vehicle,camera,camera_depth,camera_segmentation,world_2_camera,drone_civilian_ids
...
camera_depth_bp = bp_lib.find('sensor.camera.depth')
camera_segmentation_bp=bp_lib.find('sensor.camera.semantic_segmentation')
...
camera_depth_bp.set_attribute("image_size_x",str(640))
camera_depth_bp.set_attribute("image_size_y",str(380))
camera_depth_bp.set_attribute("fov",str(90))
camera_depth_bp.set_attribute("sensor_tick",str(0.1))

camera_segmentation_bp.set_attribute("image_size_x",str(640))
camera_segmentation_bp.set_attribute("image_size_y",str(380))
camera_segmentation_bp.set_attribute("fov",str(90))
camera_segmentation_bp.set_attribute("sensor_tick",str(0.1))
...
image_depth_queue = queue.Queue()
image_segmentation_queue = queue.Queue()

id,vehicle,camera,camera_depth,camera_segmentation,world_2_camera,drone_civilian_ids = connect(world,camera_bp,image_queue,image_depth_queue,image_segmentation_queue,scenic_timeout)
...
while True:
    ...
    try:
        ...
        camera.destroy()
        camera_segmentation.destroy()
        camera_depth.destroy()
        id,vehicle,camera,camera_depth,camera_segmentation,world_2_camera,drone_civilian_ids = connect(world,camera_bp,image_queue,image_depth_queue,image_segmentation_queue,scenic_timeout)
        print(drone_civilian_ids)

    image = image_queue.get()
    image_depth = image_depth_queue.get()
    image_segmentation = image_segmentation_queue.get()

    image_segmentation.convert(carla.ColorConverter.CityScapesPalette)
    image_depth.convert(carla.ColorConverter.Depth)

    img = np.reshape(np.copy(image.raw_data), (image.height, image.width, 4))
    img_depth = np.reshape(np.copy(image_depth.raw_data), (image.height, image.width, 4))
    img_segmentation = np.reshape(np.copy(image_segmentation.raw_data), (image.height, image.width, 4))
    ...
    cv2.imshow('Segmentation Camera',img_segmentation)
